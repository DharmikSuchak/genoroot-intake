...
export function Q13ProceduresGateway() {
  const value = useIntakeStore(s => s.proceduresGateway);
  const setProceduresGateway = useIntakeStore(s => s.setProceduresGateway);

  function tap(answer: boolean) {
    setProceduresGateway(value === answer ? null : answer);
  }
    ...
    <div className="flex flex-col min-h-full px-4 py-8 bg-slate-50">
      <div className="max-w-sm mx-auto w-full flex flex-col gap-6">
        <div className="flex flex-col gap-3">
          <p className="text-sm font-medium tracking-wide text-sky-600">Question 13 of 16</p>
          <h2
            className="text-xl font-semibold text-slate-800 leading-snug"
            style={{ fontFamily: 'var(--font-outfit), system-ui, sans-serif' }}
          >
            Have you had any in-clinic hair procedures?
          </h2>
          <p className="text-base text-slate-500">
            PRP, GFC, iPRF, stem cells, exosomes, hair transplant, or similar.
          </p>
        </div>
