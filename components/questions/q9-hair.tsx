...
export function Q9Hair() {
  const value = useIntakeStore(s => s.form.excess_body_facial_hair);
  const setField = useIntakeStore(s => s.setField);

  function tap(answer: boolean) {
    setField('excess_body_facial_hair', value === answer ? null : answer, 'tapped');
  }
    ...
    <div className="flex flex-col min-h-full px-4 py-8 bg-slate-50">
      <div className="max-w-sm mx-auto w-full flex flex-col gap-6">
        <div className="flex flex-col gap-3">
          <p className="text-sm font-medium tracking-wide text-sky-600">Question 9 of 16</p>
          <h2
            className="text-xl font-semibold text-slate-800 leading-snug"
            style={{ fontFamily: 'var(--font-outfit), system-ui, sans-serif' }}
          >
            Have you noticed excess body or facial hair compared to what's usual for you?
          </h2>
        </div>
