...
export function Q11Heating() {
  const value = useIntakeStore(s => s.form.habits.heating_tools_styling_chemicals);
  const setHabit = useIntakeStore(s => s.setHabit);

  function tap(answer: boolean) {
    setHabit('heating_tools_styling_chemicals', value === answer ? null : answer, 'tapped');
  }
    ...
    <div className="flex flex-col h-full px-4 py-8 bg-slate-50">
      <div className="max-w-sm mx-auto w-full flex flex-col gap-6">
        <div className="flex flex-col gap-3">
          ...
          >
            Do you regularly use heat styling tools or chemical treatments on your hair?
          </h2>
          <p className="text-base text-slate-500">
            Blow dryers, straighteners, curlers, perms, relaxers, etc.
          </p>
        </div>
