...
export function Q11Salon() {
  const value = useIntakeStore(s => s.form.habits.salon_treatments);
  const setHabit = useIntakeStore(s => s.setHabit);

  function tap(answer: boolean) {
    setHabit('salon_treatments', value === answer ? null : answer, 'tapped');
  }
    ...
    <div className="flex flex-col h-full px-4 py-8 bg-slate-50">
      <div className="max-w-sm mx-auto w-full flex flex-col gap-6">
        <div className="flex flex-col gap-3">
          ...
          >
            Do you get professional salon hair treatments?
          </h2>
          <p className="text-base text-slate-500">
            Keratin, bleaching, colouring, straightening, etc.
          </p>
        </div>
