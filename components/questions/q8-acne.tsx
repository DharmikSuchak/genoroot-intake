...
export function Q8Acne() {
  const value = useIntakeStore(s => s.form.adult_acne_oily_skin);
  const setField = useIntakeStore(s => s.setField);

  function tap(answer: boolean) {
    setField('adult_acne_oily_skin', value === answer ? null : answer, 'tapped');
  }
    ...
    <div className="flex flex-col h-full px-4 py-8 bg-slate-50">
      <div className="max-w-sm mx-auto w-full flex flex-col gap-6">
        <div className="flex flex-col gap-3">
          <p className="text-sm font-medium tracking-wide text-sky-600">Question 8 of 16</p>
          <h2
            className="text-xl font-semibold text-slate-800 leading-snug"
            style={{ fontFamily: 'var(--font-outfit), system-ui, sans-serif' }}
          >
            Do you experience adult acne or oily skin?
          </h2>
        </div>
